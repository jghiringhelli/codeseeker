/**
 * CodeMind E2E Integration Test Suite
 *
 * Tests the complete CodeMind workflow including:
...
describe('CodeMind E2E Integration Tests', () => {
  ...
  describe('Init Command', () => {
    ...
    it('should initialize project successfully', async () => {
      const result = await executeCodemind('init --no-docs', {
      ...
      // Verify .codemind folder was created
      const codemindDir = path.join(testConfig.testProjectPath, '.codemind');
      const exists = await verifyFileExists(codemindDir);
  ...
  describe('Search Toggle Edge Cases', () => {
    ...
    it('should handle help command', async () => {
      const result = await executeCodemind('--help', {
        cwd: testConfig.testProjectPath,
        timeout: 10000
      });

      expect(result.exitCode).toBe(0);
      expect(result.stdout).toMatch(/codemind|usage|options/i);
    }, 15000);

    it('should handle version command', async () => {
      const result = await executeCodemind('--version', {
        cwd: testConfig.testProjectPath,
        timeout: 10000
      });
...
async function runStandaloneTests(): Promise<void> {
  console.log('\n' + '='.repeat(70));
  console.log('CodeMind E2E Integration Test Suite');
  ...
  try {
    ...
    try {
      const result = await executeCodemind('init', {
        cwd: testConfig.testProjectPath,
        timeout: COMMAND_TIMEOUT,
        mockClaude
      });
