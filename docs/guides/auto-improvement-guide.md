...
> **User Guide** | [← Back to User Guides](README.md) | [Quick Start](#quick-start) | [Interactive Mode](#interactive-mode) | [Command Reference](#command-reference)
...
3. [Interactive Mode](#interactive-mode) ⭐ **Recommended**
...
|--------|-------------|---------|
| `--dry-run` | Analyze only, don't modify files | `--dry-run` |
| `--types <types...>` | Specific fix types | `--types duplicates centralization` |
| `--aggressiveness <level>` | Fix aggressiveness level | `--aggressiveness conservative` |
| `--output <path>` | Custom output directory for reports | `--output ./reports` |
| `--no-backup` | Skip backup creation (use with caution) | `--no-backup` |
| `--no-report` | Skip report generation | `--no-report` |
| `--interactive` | Interactive mode with Git workflow | `--interactive` |
| `--auto-approve` | Auto-approve in interactive mode | `--auto-approve` |
| `--skip-tests` | Skip running tests | `--skip-tests` |
| `--skip-git` | Skip Git workflow | `--skip-git` |
| `--verbose` | Verbose logging for debugging | `--verbose` |

### Fix Types

| Type | Description | What It Does |
|------|-------------|--------------|
| `duplicates` | Code duplication detection | Finds and refactors duplicate code blocks |
| `centralization` | Configuration centralization | Consolidates scattered configuration values |
| `dependencies` | Dependency optimization | Resolves circular dependencies |
| `quality` | Code quality improvements | Applies style and structure enhancements |
| `all` | All available fixes | Runs all improvement types |

### Aggressiveness Levels

| Level | Description | Recommended For |
|-------|-------------|-----------------|
| `conservative` | Safe, minimal changes | Production codebases, critical systems |
| `moderate` | Balanced approach | Most projects, regular maintenance |
| `aggressive` | Comprehensive improvements | Legacy modernization, major refactoring |
...
# Interactive with auto-approval for CI/CD
npx codemind auto-fix ./my-project --interactive --auto-approve
```
