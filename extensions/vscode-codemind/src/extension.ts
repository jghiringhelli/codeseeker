 ...
 *
 * Keeps the CodeMind semantic index in sync with file changes.
 * Automatically notifies the CodeMind MCP server when files are
...
export function activate(context: vscode.ExtensionContext): void {
  console.log('CodeMind extension activating...');
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('codemind.syncNow', cmdSyncNow),
    vscode.commands.registerCommand('codemind.fullReindex', cmdFullReindex),
    vscode.commands.registerCommand('codemind.showStatus', cmdShowStatus),
    vscode.commands.registerCommand('codemind.toggleAutoSync', cmdToggleAutoSync)
  );
...
export function deactivate(): void {
  console.log('CodeMind extension deactivating...');
}
...
function getConfiguration(): {
  ...
  return {
    ...
    mcpCommand: config.get('mcpCommand', 'codemind'),
...
async function handleFileChanges(changes: FileChange[]): Promise<void> {
  ...
  try {
    ...
    vscode.window.showErrorMessage(`CodeMind sync failed: ${message}`);
  }
}
  ...
  if (result.success) {
    ...
    if (result.durationMs && result.durationMs > 5000) {
      vscode.window.showInformationMessage(
        `CodeMind: ${message} in ${(result.durationMs / 1000).toFixed(1)}s`
      );
    }
  } else {
    statusBar.setStatus('error', result.error || 'Unknown error');
    vscode.window.showErrorMessage(`CodeMind sync failed: ${result.error}`);
  }
}
...
async function cmdSyncNow(): Promise<void> {
  ...
  if (changes.length === 0) {
    vscode.window.showInformationMessage('CodeMind: No pending changes to sync');
    return;
  }
...
async function cmdFullReindex(): Promise<void> {
  ...
  const confirm = await vscode.window.showWarningMessage(
    'This will rebuild the entire CodeMind index for this project. Continue?',
    'Yes, Reindex',
    'Cancel'
  );
  ...
  try {
    ...
    vscode.window.showErrorMessage(`CodeMind reindex failed: ${message}`);
  }
}
...
async function cmdShowStatus(): Promise<void> {
  ...
  const items: vscode.QuickPickItem[] = [
    ...
    },
    {
      ...
      detail: 'Clear and rebuild the entire CodeMind index for this project'
    },
    {
    ...
    },
    {
      ...
      detail: 'Open the CodeMind MCP output channel'
    }
  ];
...
async function cmdToggleAutoSync(): Promise<void> {
  ...
  vscode.window.showInformationMessage(
    `CodeMind auto-sync ${!current ? 'enabled' : 'disabled'}`
  );
}