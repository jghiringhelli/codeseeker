...
exports.VoiceStrategyInterface = void 0;
const events_1 = require("events");
const fs = __importStar(require("fs/promises"));
class VoiceStrategyInterface extends events_1.EventEmitter {
    logger;
    projectKB;
    knowledgeRepo;
    activeSessions = new Map();
    voiceSettings;
    speechRecognition; // Would be WebSpeechAPI or similar
    speechSynthesis;
    nlpProcessor; // Would be actual NLP service
    sessionsPath;
    constructor(logger, projectKB, knowledgeRepo, sessionsPath = './voice-sessions') {
        super();
        this.logger = logger;
    ...
    async startStrategicConversation(userId, topic = 'Project Strategy Review') {
        ...
        await this?.addOrchestratorResponse(sessionId, this?.generateGreeting(topic), 'question', 'strategy');
        this.logger.info(`Strategic conversation started: ${sessionId} - ${topic}`);
        this?.emit('conversation-started', { sessionId, topic });
        return sessionId;
    }
    async processUserInput(sessionId, input, isVoice = false) {
        ...
        const orchestratorTurn = await this?.addOrchestratorResponse(sessionId, response.content, response.intent.type, response.intent.category);
    ...
    async addUserInput(sessionId, input, isVoice) {
        const session = this.activeSessions?.get(sessionId);
    ...
    async addOrchestratorResponse(sessionId, content, intentType, intentCategory) {
        const session = this.activeSessions?.get(sessionId);
        const turn = {
            id: `turn-${Date?.now()}`,
            timestamp: new Date(),
    ...
    async generateOrchestratorResponse(session, userTurn) {
    ...
    async handleStrategicQuestion(userTurn, context) {
        // Search knowledge repository for relevant information
        const searchResults = await this.knowledgeRepo?.searchKnowledge(userTurn.content, { limit: 3, useHybrid: true });
        let response = '';
    ...
    async handleStrategicConcern(userTurn, context, recentInsights) {
    ...
    async handleStrategicIdea(userTurn, context) {
    ...
    async handleDecisionMaking(userTurn, context, pendingDecisions) {
    ...
    async handleGeneralDiscussion(userTurn, context) {
        let response = '';
    ...
    async analyzeSentiment(input) {
        ...
        if (excitedWords?.some(word => lowerInput?.includes(word)))
            return 'excited';
        if (concernedWords?.some(word => lowerInput?.includes(word)))
            return 'concerned';
    ...
    async processForInsights(session, turn) {
...
exports.VoiceStrategyInterface = VoiceStrategyInterface;
//# sourceMappingURL=voice-strategy-interface.js.map