...
class LLMIntentionDetector {
    ...
    async analyzeIntention(userInput, projectContext) {
        if (!this.enableLLM) {
            return null;
        }
        try {
            const prompt = this.buildAnalysisPrompt(userInput, projectContext);
            const response = await this.claudeIntegration.executeClaudeCode(prompt, '', {
    ...
    buildAnalysisPrompt(userInput, projectContext) {
...
PROJECT CONTEXT: ${projectContext ? JSON.stringify(projectContext, null, 2) : 'No specific project context available'}
