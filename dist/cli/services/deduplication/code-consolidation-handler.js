...
exports.CodeConsolidationHandler = void 0;
const logger_1 = require("../../../utils/logger");
class CodeConsolidationHandler {
    logger = logger_1.Logger.getInstance();
    /**
     * Create consolidation plans for duplicate groups
     */
    async createConsolidationPlans(report) {
     ...
     */
    async executeConsolidations(plans, claudeOrchestrator, projectPath) {
     ...
     */
    async createPlanForGroup(group) {
     ...
     */
    async executeConsolidationPlan(plan, claudeOrchestrator, projectPath) {
        try {
...
`;
    ...
    createExactFunctionExtractionInstructions(group) {
        const mainChunk = group.chunks[0];
    ...
    createExactClassExtractionInstructions(group) {
        const mainChunk = group.chunks[0];
    ...
    createUtilityExtractionInstructions(group) {
    ...
    createSemanticMergeInstructions(group) {
        const mainChunk = group.chunks[0];
    ...
    createSemanticUtilityInstructions(group) {
    ...
    createBaseClassInstructions(group) {
        const mainChunk = group.chunks[0];
    ...
    createStructuralUtilityInstructions(group) {
    ...
    // Helper methods
    suggestUtilityLocation(referenceFile) {
    ...
    suggestBaseClassLocation(referenceFile) {
        const path = require('path');
        const dir = path.dirname(referenceFile);
        return path.join(dir, 'base', 'base-classes.ts');
    }
    estimateLinesReduced(plan) {
    ...
    createConsolidationSummary(results, errors) {
...
exports.CodeConsolidationHandler = CodeConsolidationHandler;
exports.default = CodeConsolidationHandler;
//# sourceMappingURL=code-consolidation-handler.js.map