...
class SmartDedupDetector {
    ...
    async updateEmbeddingsForChangedFiles(projectId, changedFiles) {
        ...
        if (filesToUpdate.length === 0)
            return;
        // Use SemanticSearchManager to regenerate embeddings
        const SemanticSearchManager = (await Promise.resolve().then(() => __importStar(require('../../../shared/managers/semantic-search-manager')))).default;
        const semanticManager = new SemanticSearchManager();
        // Re-initialize for changed files
        await semanticManager.initializeProject(projectId, filesToUpdate, (progress, file) => {
            console.log(`    Updating: ${file} (${progress}%)`);
        });
    }
    /**
