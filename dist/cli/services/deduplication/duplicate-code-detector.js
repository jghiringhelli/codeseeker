...
class DuplicateCodeDetector {
    ...
    async extractCodeChunks(projectPath, config) {
        const chunks = [];
        const { ProjectFileScanner } = await Promise.resolve().then(() => __importStar(require('../file-scanner/project-file-scanner')));
        const { TreeSitterSemanticBuilder } = await Promise.resolve().then(() => __importStar(require('../semantic-graph/tree-sitter-semantic-builder')));
        // Get all code files
