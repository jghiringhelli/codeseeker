...
class AssumptionDetector {
    ...
    constructor(enableLLM = true) {
        this.llmDetector = new llm_intention_detector_1.LLMIntentionDetector(enableLLM);
        this.enableLLM = enableLLM;
    }
    /**
     * Get the full LLM intention analysis (new enhanced method)
     */
    async analyzeIntentionWithLLM(userInput, projectContext) {
        if (!this.enableLLM) {
            return null;
        }
        try {
            return await this.llmDetector.analyzeIntention(userInput, projectContext);
        }
        catch (error) {
     ...
     */
    async analyzeRequest(userInput, projectContext) {
        // Try LLM-based analysis first
        if (this.enableLLM) {
            try {
                const llmAnalysis = await this.llmDetector.analyzeIntention(userInput, projectContext);
                if (llmAnalysis) {
     ...
     */
    convertLLMAnalysisToAmbiguityAnalysis(llmAnalysis) {
        const assumptions = llmAnalysis.assumptions.map(assumption => ({
        ...
        const clarificationNeeded = llmAnalysis.ambiguities.map(amb => amb.clarificationNeeded);
        const suggestedQuestions = llmAnalysis.ambiguities.flatMap(amb => amb.options);
     ...
     */
    mapLLMCategoryToLegacy(llmCategory) {
        const categoryMap = {
            'scope': 'scope',
            'approach': 'approach',
            'data': 'data',
     ...
     */
    mapLLMConfidenceToLegacy(confidence) {
        if (confidence >= 0.8)
            return 'high';
        if (confidence >= 0.5)
            return 'medium';
     ...
     */
    analyzeRequestKeywordBased(userInput, projectContext) {
