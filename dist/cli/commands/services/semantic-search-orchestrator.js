...
class SemanticSearchOrchestrator {
     ...
     */
    async performHybridSearch(query, projectPath) {
     ...
     */
    async searchByKeywords(query, projectPath) {
        if (!this.projectId)
            return [];
        try {
            const postgres = await this.dbConnections.getPostgresConnection();
     ...
     */
    async searchByKeywordsILIKE(query, projectPath) {
        if (!this.projectId)
            return [];
        try {
            const postgres = await this.dbConnections.getPostgresConnection();
     ...
     */
    async searchByFilePath(query, projectPath) {
        if (!this.projectId)
            return [];
        try {
            const postgres = await this.dbConnections.getPostgresConnection();
     ...
     */
    async searchByVectorSimilarity(query, projectPath) {
        try {
            const postgres = await this.dbConnections.getPostgresConnection();
            // First, check if we have embeddings for this project
            let embeddingCount = 0;
            if (this.projectId && this.isValidUUID(this.projectId)) {
                const countResult = await postgres.query('SELECT COUNT(*) as count FROM semantic_search_embeddings WHERE project_id = $1', [this.projectId]);
