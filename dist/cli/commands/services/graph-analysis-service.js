...
class GraphAnalysisService {
     ...
     */
    async lookupNodesEmbedded(semanticResults) {
        const graphStore = await this.initializeGraphStore();
        if (!graphStore || !this.projectId)
            return [];
        const entities = [];
        ...
        for (const fileNode of fileNodes) {
            ...
            const isMatch = filePaths.some(fp => normalizedPath.endsWith(fp) ||
                    ...
                    if (edge.type === 'contains') {
                        ...
                        if (entityNode) {
                            entities.push({
                                ...
                                filePath: fileNode.filePath,
                                startLine: entityNode.properties?.startLine || 1,
                                endLine: entityNode.properties?.endLine || 1
                            });
                        }
                    }
                }
     ...
     */
    async lookupNodesNeo4j(semanticResults) {
        const driver = await this.dbConnections.getNeo4jConnection();
        const session = driver.session();
        try {
     ...
     */
    async getRelationshipsEmbedded(nodes) {
        const graphStore = await this.initializeGraphStore();
        if (!graphStore)
            return [];
        const relationships = [];
            ...
            for (const edge of edges) {
                ...
                if (nodeIdSet.has(edge.target)) {
                    ...
                    relationships.push({
                        ...
                        fromMethod: edge.properties?.fromMethod,
                        toMethod: edge.properties?.toMethod,
     ...
     */
    async getRelationshipsNeo4j(nodes) {
        const driver = await this.dbConnections.getNeo4jConnection();
        const session = driver.session();
        try {
     ...
     */
    async expandOneHopEmbedded(seedNodes, options) {
        const graphStore = await this.initializeGraphStore();
        if (!graphStore)
            return { expandedNodes: [], expandedRelationships: [] };
        const expandedNodes = [];
        ...
        for (const seedNode of seedNodes) {
            ...
            for (const edge of edges) {
                ...
                if (!options.relationshipTypes.includes(edgeTypeUpper))
                    continue;
                // Skip if this is a seed node
                if (seedIdSet.has(edge.target))
                    continue;
                // Skip if we've already seen this neighbor
                if (seenNeighborIds.has(edge.target))
                    continue;
                ...
                expandedNodes.push({
                    ...
                    startLine: neighborNode.properties?.startLine || 1,
                    endLine: neighborNode.properties?.endLine || 1
                });
     ...
     */
    async expandOneHopNeo4j(seedNodes, options) {
        const expandedNodes = [];
        const expandedRelationships = [];
        const seenNeighborIds = new Set();
        const driver = await this.dbConnections.getNeo4jConnection();
