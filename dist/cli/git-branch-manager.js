...
class GitBranchManager {
    ...
    async createFeatureBranch(workflowId, description) {
        try {
            ...
            try {
                ...
                this.logger.warn('⚠️ Could not pull latest changes (no remote or network issue)');
            }
    ...
    async commitChanges(message, files) {
        try {
            ...
            this.logger.info(`✅ Committed changes: ${hash} (${filesChanged} files)`);
            return {
                hash,
                message,
    ...
    async commitAndMerge(branchName, finalMessage) {
        try {
            this.logger.info(`🔀 Merging branch ${branchName} to main`);
            ...
            this.logger.info(`✅ Successfully merged and cleaned up ${branchName}`);
        }
        catch (error) {
            this.logger.error(`❌ Failed to merge branch: ${error.message}`);
    ...
    async rollbackBranch(branchName) {
        try {
            ...
            this.logger.info(`✅ Rolled back and cleaned up ${branchName}`);
        }
        catch (error) {
            this.logger.error(`❌ Failed to rollback branch: ${error.message}`);
