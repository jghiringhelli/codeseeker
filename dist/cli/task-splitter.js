...
class TaskSplitter {
    ...
    buildTaskSplitPrompt(splitRequest) {
        const { userRequest, intent, context } = splitRequest;
        const filesList = context.primaryFiles.slice(0, 8).map(f => `- ${f.filePath} (relevance: ${f.relevanceScore})`).join('\n');
        const relatedFilesList = context.relatedFiles.slice(0, 12).map(f => `- ${f.filePath} (${f.relationshipType})`).join('\n');
...
Provide only the JSON response, no additional text.`;
    ...
    async processWithClaude(prompt) {
        ...
        try {
            // Use ClaudeCodeIntegration with built-in chunking system
            const result = await this.claudeIntegration.executeClaudeCode(prompt, 'Task breakdown analysis for complex user request', {
                projectPath: process.env.CODEMIND_USER_CWD || process.cwd(),
                maxTokens: 4000
            });
            if (!result.success) {
                this.logger.warn(`Task split Claude execution failed: ${result.error}, using fallback`);
                return this.generateMockClaudeTaskSplit(prompt);
            }
            this.logger.debug(`Claude task breakdown response received (${result.tokensUsed} tokens used)`);
            return result.data;
        }
        catch (error) {
            this.logger.warn(`Task split Claude processing failed: ${error.message}, using fallback`);
            // Fall back to mock response if Claude Code fails
            return this.generateMockClaudeTaskSplit(prompt);
        }
    }
    generateMockClaudeTaskSplit(prompt) {
    ...
    parseTaskSplitResponse(response, originalQuery) {
        try {
    ...
    optimizeTaskOrder(tasks, context) {
    ...
    validateTasks(tasks, intent, context) {
        ...
        const validatedTasks = tasks.filter(task => {
            ...
            const filesExistInContext = task.files.some(file => context.primaryFiles.some(cf => cf.filePath.includes(file)) ||
    ...
    generateFallbackTasks(request, intent, context) {
    ...
    generateFallbackTaskSplit(query) {
        return {
            subTasks: [{
                    id: 'fallback_task',
