...
(function (DuplicationType) {
    DuplicationType["EXACT"] = "exact";
    DuplicationType["STRUCTURAL"] = "structural";
    DuplicationType["SEMANTIC"] = "semantic";
    DuplicationType["RENAMED"] = "renamed";
})(DuplicationType || (exports.DuplicationType = DuplicationType = {}));
var RefactoringApproach;
(function (RefactoringApproach) {
    RefactoringApproach["EXTRACT_FUNCTION"] = "extract_function";
    RefactoringApproach["EXTRACT_CLASS"] = "extract_class";
    RefactoringApproach["EXTRACT_UTILITY"] = "extract_utility";
    RefactoringApproach["USE_INHERITANCE"] = "use_inheritance";
    RefactoringApproach["APPLY_STRATEGY_PATTERN"] = "apply_strategy_pattern";
    RefactoringApproach["CONSOLIDATE_CONFIGURATION"] = "consolidate_configuration";
})(RefactoringApproach || (exports.RefactoringApproach = RefactoringApproach = {}));
var EffortEstimate;
(function (EffortEstimate) {
    EffortEstimate["LOW"] = "low";
    EffortEstimate["MEDIUM"] = "medium";
    EffortEstimate["HIGH"] = "high";
    EffortEstimate["VERY_HIGH"] = "very_high"; // > 8 hours
})(EffortEstimate || (exports.EffortEstimate = EffortEstimate = {}));
class DuplicationDetector {
    ...
    async findDuplicates(request) {
        ...
        try {
            ...
            // Find duplications
            const duplicateGroups = await this.findDuplicateGroups(codeBlocks, request.similarityThreshold, request.includeSemantic);
    ...
    async getProjectFiles(projectPath, includePatterns, excludePatterns) {
        ...
        return await (0, fast_glob_1.glob)(includePatterns || defaultPatterns, {
            cwd: projectPath,
    ...
    async extractCodeBlocks(filePath) {
        ...
        for (const symbol of astResult.symbols) {
            if (symbol.type === 'function' || symbol.type === 'method' || symbol.type === 'class') {
                const blockContent = this.extractBlockContent(lines, symbol.location.line - 1, symbol.location.endLine ? symbol.location.endLine - 1 : symbol.location.line + 10);
    ...
    extractBlockContent(lines, startIndex, endIndex) {
    ...
    calculateStructuralFingerprint(content, astResult) {
        ...
        const controlFlowHash = (0, crypto_1.createHash)('md5')
            .update(controlFlowElements.join(','))
            .digest('hex')
            .substring(0, 8);
        const dependencyHash = (0, crypto_1.createHash)('md5')
    ...
    async findDuplicateGroups(blocks, threshold, includeSemantic) {
        const groups = [];
        ...
        for (let i = 0; i < blocks.length; i++) {
            ...
            for (let j = i + 1; j < blocks.length; j++) {
                if (processed.has(blocks[j].hash))
                    continue;
                const similarity = await this.calculateSimilarity(blocks[i], blocks[j], includeSemantic);
    ...
    async calculateSimilarity(block1, block2, includeSemantic) {
        // Exact match
        ...
        return (structuralSimilarity * weights.structural +
    ...
    async calculateSemanticSimilarity(block1, block2) {
    ...
    determineDuplicationType(blocks) {
        ...
        const structurallyIdentical = blocks.every(block => block.structure.controlFlowHash === firstStructure.controlFlowHash &&
    ...
    calculateAverageTokenSimilarity(blocks) {
    ...
    calculateGroupSimilarity(blocks) {
        if (blocks.length < 2)
            return 1.0;
        let totalSimilarity = 0;
        let comparisons = 0;
    ...
    async generateRefactoringAdvice(group) {
    ...
    getRefactoringDescription(approach, group) {
        const locations = group.locations.length;
        const lines = group.metadata.linesOfCode;
        const descriptions = {
            [RefactoringApproach.EXTRACT_FUNCTION]: `Extract the duplicated ${lines}-line code block into a reusable function. This will eliminate ${locations} duplicates and improve maintainability.`,
            [RefactoringApproach.EXTRACT_CLASS]: `The large duplicated block (${lines} lines) should be extracted into a separate class. This will improve code organization and reduce coupling.`,
            [RefactoringApproach.EXTRACT_UTILITY]: `Create a utility module for this commonly duplicated functionality (found in ${locations} locations). This will centralize the logic and make it reusable.`,
            [RefactoringApproach.USE_INHERITANCE]: `Consider using inheritance or composition to eliminate this duplication. The similar structure suggests a common base class could be beneficial.`,
            [RefactoringApproach.APPLY_STRATEGY_PATTERN]: `The complex duplicated logic (complexity: ${group.metadata.complexity}) could benefit from the Strategy pattern to reduce duplication while maintaining flexibility.`,
            [RefactoringApproach.CONSOLIDATE_CONFIGURATION]: `This appears to be configuration or constant duplication. Consider centralizing these values in a configuration file or constants module.`
        };
        return descriptions[approach] || 'Consider refactoring to eliminate duplication.';
    }
    getRefactoringSteps(approach, group) {
        const baseSteps = [
    ...
    calculateMaintainabilityImpact(approach, group) {
    ...
    calculateRiskLevel(approach, group) {
    ...
    generateExtractionExample(group) {
...
const result = ${functionName}(arguments);
    ...
    suggestFunctionName(codeSnippet) {
        // Simple name suggestion based on code content
        if (codeSnippet.includes('validate'))
            return 'validateInput';
        if (codeSnippet.includes('format'))
            return 'formatData';
        if (codeSnippet.includes('calculate'))
            return 'calculateValue';
        if (codeSnippet.includes('transform'))
            return 'transformData';
        if (codeSnippet.includes('process'))
            return 'processData';
...
exports.default = DuplicationDetector;
