...
class GitBranchManager {
     ...
     */
    async createFeatureBranch(userRequest, requestId) {
     ...
     */
    async createSnapshot(branchName, description, validationPassed, changedFiles) {
        try {
     ...
     */
    async performRollback(branchName, options) {
        try {
            ...
            await this.createSnapshot(branchName, `Rollback: ${message}`, false, restoredFiles);
     ...
     */
    async cherryPickChanges(sourceBranch, targetBranch, commitHashes) {
        try {
     ...
     */
    async mergeFeatureBranch(branchName, squashCommits = true) {
        try {
            const featureBranch = this.activeBranches.get(branchName);
            if (!featureBranch) {
                throw new Error(`Feature branch ${branchName} not found`);
     ...
     */
    async cleanupBranches(maxAge = 7, // days
    keepMerged = false) {
        const cleaned = [];
        const kept = [];
        try {
            ...
            for (const [branchName, featureBranch] of this.activeBranches) {
                const shouldCleanup = featureBranch.created < cutoffDate &&
    ...
    async performCompleteRollback(featureBranch, options) {
    ...
    async performPartialRollback(featureBranch, options) {
        let targetCommit;
    ...
    async performSelectiveRollback(featureBranch, options) {
    ...
    generateBranchName(userRequest, requestId) {
    ...
    generateCommitMessage(description, validationPassed, changedFiles) {
    ...
    async getParentBranchCommit(parentBranch) {
        const { stdout } = await this.execGit(`rev-parse ${parentBranch}`);
        return stdout.trim();
    }
    getAllChangedFiles(snapshots) {
    ...
    async getChangedFilesBetweenCommits(from, to) {
        try {
