...
const intelligent_tool_selector_1 = require("./intelligent-tool-selector");
const hybrid_file_discovery_1 = require("./hybrid-file-discovery");
...
class EnhancedContextProvider {
     ...
     */
    async generateEnhancedContext(request) {
     ...
     */
    mapIntentForFileDiscovery(intent) {
        switch (intent) {
     ...
     */
    async selectToolsForContext(request, fileContext) {
     ...
     */
    async runToolAnalyses(request, toolSelection, fileContext) {
        const analyses = [];
     ...
     */
    async generateContextSections(toolResults, request) {
        const sections = [];
     ...
     */
    async generateToolContextContent(result, request) {
        const sections = [];
     ...
     */
    calculateTokenWeight(selectionInfo, metadata, request) {
        ...
        if (request.intent) {
            ...
            const boosts = intentBoosts[request.intent];
            if (boosts && boosts[metadata.category]) {
                weight += boosts[metadata.category];
            }
        }
        // Trust level boost
     ...
     */
    async optimizeContextForTokens(sections, tokenBudget) {
     ...
     */
    async generateCrossToolInsights(toolResults) {
        const insights = [];
     ...
     */
    extractCommonThemes(recommendations) {
     ...
     */
    detectRecommendationConflicts(toolResults) {
     ...
     */
    async generateRecommendedActions(toolResults, request) {
        const actions = [];
        const allRecommendations = toolResults.flatMap(r => r.result.recommendations || []);
        ...
        toolResults.forEach(toolResult => {
            ...
            toolResult.result.recommendations?.forEach((rec) => {
...
//# sourceMappingURL=enhanced-context-provider.js.map