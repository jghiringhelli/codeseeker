...
class IntelligentCycleFeatures {
    ...
    async checkSemanticDuplication(projectPath, userIntent, changedFiles) {
        ...
        try {
            ...
            if (this.initialized) {
                try {
                    const semanticMatches = await this.findSemanticMatches(intentAnalysis.intendedFunctionality, projectPath);
            ...
            const patternMatches = await this.findPatternMatches(intentAnalysis.intendedFunctionality, intentAnalysis.detectedPatterns, projectPath);
            ...
            if (result.hasDuplicates) {
                result.recommendations = this.generateDuplicationRecommendations(result.existingImplementations, intentAnalysis);
     ...
     */
    async findSemanticMatches(functionality, projectPath) {
        const matches = [];
        try {
     ...
     */
    async findVectorSimilarityMatches(functionality) {
        const matches = [];
        try {
     ...
     */
    async findRelationshipMatches(functionality) {
        const matches = [];
        try {
            ...
            for (const result of relationshipResults) {
                ...
                if (nodeProps.nodeType === 'function' || nodeProps.nodeType === 'class') {
                    matches.push({
                        ...
                        codeSnippet: await this.extractCodeSnippet(nodeProps.file, nodeProps.startLine || 1, nodeProps.endLine || 10),
                        lineRange: {
     ...
     */
    async findSemanticMatchesLegacy(functionality, projectPath) {
        const matches = [];
        try {
            ...
            for (const result of semanticResults) {
                ...
                if (nodeProps.nodeType === 'function' || nodeProps.nodeType === 'class') {
                    const snippet = nodeProps.file ?
                        await this.extractCodeSnippet(nodeProps.file, nodeProps.startLine || 1, nodeProps.endLine || nodeProps.startLine + 10 || 10) : 'Code snippet unavailable';
                    matches.push({
                        file: nodeProps.file || 'Unknown file',
                        function: nodeProps.nodeType === 'function' ? nodeProps.name : undefined,
                        class: nodeProps.nodeType === 'class' ? nodeProps.name : undefined,
     ...
     */
    async findPatternMatches(functionality, patterns, projectPath) {
        const matches = [];
        try {
            // Find relevant files
            const files = await (0, fast_glob_1.glob)('**/*.{ts,js}', {
                cwd: projectPath,
     ...
     */
    generateSearchPatterns(functionality, patterns) {
     ...
     */
    extractSnippetFromLines(lines, centerLine, context) {
     ...
     */
    generateDuplicationRecommendations(implementations, intentAnalysis) {
        const recommendations = [];
        // Sort by similarity
        const sorted = implementations.sort((a, b) => b.similarity - a.similarity);
        const topMatch = sorted[0];
        if (topMatch.similarity > 0.8) {
            recommendations.push(`⚠️ High similarity (${Math.round(topMatch.similarity * 100)}%) with existing code in ${topMatch.file}`);
            recommendations.push(`Consider extending or refactoring the existing ${topMatch.function || topMatch.class} instead`);
        }
        else if (topMatch.similarity > 0.6) {
            recommendations.push(`Similar functionality exists in ${topMatch.file} (${Math.round(topMatch.similarity * 100)}% match)`);
            recommendations.push(`Consider reviewing the existing implementation for reusable patterns`);
        }
        // Suggest better names if conflicts exist
        if (implementations.some(impl => intentAnalysis.suggestedNames.some(name => impl.function?.toLowerCase().includes(name.toLowerCase()) ||
            impl.class?.toLowerCase().includes(name.toLowerCase())))) {
            recommendations.push(`Suggested alternative names: ${intentAnalysis.suggestedNames.join(', ')}`);
        }
        // Architecture recommendations
        if (intentAnalysis.architecturalConcerns.length > 0) {
            recommendations.push(`Consider these architectural concerns: ${intentAnalysis.architecturalConcerns.join(', ')}`);
        }
    ...
    async performSmartSecurity(projectPath, changedFiles, userIntent) {
        ...
        try {
            ...
            result.recommendations = this.generateSecurityRecommendations(result.vulnerabilities, userIntent);
        }
        catch (error) {
     ...
     */
    getContextualSecurityPatterns(userIntent) {
        const patterns = [];
        const lowerIntent = userIntent.toLowerCase();
     ...
     */
    async scanForVulnerabilities(content, file, contextPatterns) {
     ...
     */
    getSeverityFromPattern(description) {
        const lower = description.toLowerCase();
        if (lower.includes('injection') || lower.includes('xss'))
            return 'critical';
        if (lower.includes('weak') || lower.includes('insecure'))
            return 'medium';
        return 'low';
    }
    /**
     ...
     */
    generateSecurityRecommendations(vulnerabilities, userIntent) {
     ...
     */
    getSuggestionForVulnerability(type) {
        const suggestions = {
