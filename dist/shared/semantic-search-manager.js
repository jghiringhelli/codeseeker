...
class SemanticSearchManager {
    ...
    EMBEDDING_DIMENSIONS = 384; // OpenAI text-embedding-3-small standard
    constructor() {
     ...
     */
    async initializeProject(projectId, files, progressCallback) {
        ...
        try {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                try {
                    progressCallback?.(Math.round((i / files.length) * 100), filePath, 'Processing file content and generating chunks...');
     ...
     */
    async updateFiles(projectId, modifiedFiles) {
    ...
    async processFileForInit(projectId, filePath) {
        try {
    ...
    async storeChunksWithEmbeddings(projectId, chunks) {
        const pgClient = await this.dbConnections.getPostgresConnection();
    ...
    async searchDatabase(query) {
        try {
            ...
            const params = [embeddingVector, query.minSimilarity || 0.3];
            let paramIndex = 2;
            if (query.projectId) {
    ...
    async searchFallback(query) {
        ...
        if (queryLower.includes('create') || queryLower.includes('generate')) {
            fallbackFiles.push('client/src/App.tsx', 'server/index.js', 'package.json', 'README.md', '.codemind/codemind.md');
        }
    ...
    extractFileMetadata(filePath, content, significance) {
        return {
    ...
    determineChunkSignificance(content) {
    ...
    rankAndLimitResults(results, maxResults) {
     ...
     */
    extractCodeEntities(content, lines) {
        const entities = [];
     ...
     */
    createLogicalChunks(filePath, content, fileHash, lines, startIndex) {
        const chunks = [];
...
exports.SemanticSearchManager = SemanticSearchManager;
exports.default = SemanticSearchManager;
//# sourceMappingURL=semantic-search-manager.js.map