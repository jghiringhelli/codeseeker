...
exports.PostgreSQLAnalyticsDatabase = void 0;
const logger_1 = require("../utils/logger");
const database_config_1 = require("../config/database-config");
class PostgreSQLAnalyticsDatabase {
    dbConnections;
    logger;
    initialized = false;
    constructor(projectId) {
        this.logger = new logger_1.Logger(logger_1.LogLevel.INFO, 'PostgreSQLAnalyticsDatabase');
    ...
    async insertPerformanceMetricsBatch(metrics) {
        if (metrics.length === 0)
            return;
        await this.initialize();
        const client = await this.dbConnections.getPostgresConnection();
      ...
      VALUES ${values}`;
        ...
        metrics.forEach(metric => {
            params.push(metric.project_id, metric.tool_name, metric.execution_time, metric.cache_hit_rate, metric.memory_usage || null, metric.timestamp, JSON.stringify(metric.metadata || {}));
        });
        await client.query(query, params);
    ...
    async exportToCSV(tableName, outputPath, projectId) {
        await this.initialize();
        const client = await this.dbConnections.getPostgresConnection();
...
exports.PostgreSQLAnalyticsDatabase = PostgreSQLAnalyticsDatabase;
//# sourceMappingURL=postgresql-analytics-database.js.map