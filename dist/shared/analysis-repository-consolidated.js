...
class ConsolidatedAnalysisRepository {
     ...
     */
    async deleteEmbeddingsForFiles(projectId, filePaths) {
        if (filePaths.length === 0)
            return 0;
        try {
            const result = await this.query('DELETE FROM semantic_search_embeddings WHERE project_id = $1 AND file_path = ANY($2)', [projectId, filePaths]);
