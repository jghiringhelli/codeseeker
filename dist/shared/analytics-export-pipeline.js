...
class AnalyticsExportPipeline {
    ...
    analyticsDb;
    config;
    lastExportTimestamp = new Map();
    constructor(pgPool, analyticsDb, config) {
        this.logger = new logger_1.Logger(logger_1.LogLevel.INFO, 'AnalyticsExportPipeline');
        this.pgPool = pgPool;
    ...
    async exportPerformanceMetrics(incremental = false) {
    ...
    `;
        ...
        if (!incremental)
            query += ` LIMIT ${this.config.batchSize}`;
        try {
    ...
    async exportCodeQualityMetrics(incremental = false) {
    ...
    async exportFileChangeEvents(incremental = false) {
    ...
    async triggerExport(type = 'incremental') {
...
//# sourceMappingURL=analytics-export-pipeline.js.map