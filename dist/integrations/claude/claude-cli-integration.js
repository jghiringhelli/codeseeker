...
class ClaudeCodeIntegration {
    ...
    constructor() {
        ...
        this.conversationManager = new conversation_manager_1.ClaudeConversationManager();
    }
    /**
    ...
    async executeClaudeCode(prompt, context, options = {}) {
        try {
            console.log(`🤖 Executing Claude Code with centralized command processor...`);
            // Combine prompt and context for processing
            ...
            const result = await command_processor_1.CommandProcessor.executeClaudeCode(processedPrompt, {
            ...
            return {
                success: true,
                data: result.data,
                tokensUsed: (result.tokensUsed || 0) + chunkResult.tokensUsed,
...
Response:`;
        try {
            const response = await command_processor_1.CommandProcessor.executeClaudeCode(intentPrompt, {
...
Response:`;
    ...
    async buildEnhancedContext(userRequest, projectPath, relevantFiles, intent) {
        const { ContextBuilder } = await Promise.resolve().then(() => __importStar(require('../../cli/integration/context-builder')));
    ...
    async decomposeIntoTasks(userRequest, intent) {
        const { TaskDecomposer } = await Promise.resolve().then(() => __importStar(require('../../cli/integration/task-decomposer')));
        const taskDecomposer = new TaskDecomposer();
        return await taskDecomposer.decompose(userRequest, intent);
    }
    async executeTask(task, context) {
        const { TaskExecutor } = await Promise.resolve().then(() => __importStar(require('../../cli/integration/task-executor')));
        const taskExecutor = new TaskExecutor();
        return await taskExecutor.execute(task, context);
    }
    async performQualityCheck(result) {
        const { QualityChecker } = await Promise.resolve().then(() => __importStar(require('../../cli/quality-checker')));
        const qualityChecker = new QualityChecker();
        return await qualityChecker.check(result);
    }
    async improveTaskResult(result, qualityCheck) {
        const { ResultImprover } = await Promise.resolve().then(() => __importStar(require('../../cli/integration/result-improver')));
    ...
    async integrateResults(results) {
        const { ResultIntegrator } = await Promise.resolve().then(() => __importStar(require('../../cli/integration/result-integrator')));
    ...
    async updateKnowledgeBase(analysis) {
        const { KnowledgeBaseUpdater } = await Promise.resolve().then(() => __importStar(require('../../cli/integration/knowledge-base-updater')));
    ...
    async getProjectStructure(projectPath) {
        const { ProjectStructureAnalyzer } = await Promise.resolve().then(() => __importStar(require('../../cli/analyzers/project-structure-analyzer')));
    ...
    async getKeyConfigurations(projectPath) {
        const { ConfigurationAnalyzer } = await Promise.resolve().then(() => __importStar(require('../../cli/analyzers/configuration-analyzer')));
    ...
    async getRecentChanges(projectPath) {
        const { ChangeAnalyzer } = await Promise.resolve().then(() => __importStar(require('../../cli/analyzers/change-analyzer')));
    ...
    async getProjectDocumentation(projectPath) {
        const { DocumentationAnalyzer } = await Promise.resolve().then(() => __importStar(require('../../cli/analyzers/documentation-analyzer')));
