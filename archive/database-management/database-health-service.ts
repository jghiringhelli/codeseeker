...
import { DatabaseSchemaManager } from '../../../../services/managers/database-schema-manager';
...
export class DatabaseHealthService {
  ...
  constructor() {
    ...
    this.schemaManager = new DatabaseSchemaManager(this.connections);
  }
    ...
    try {
      ...
      if (repairResult.success) {
        if (repairResult.tablesCreated.length > 0) {
          console.log(Theme.colors.success(`✓ Created tables: ${repairResult.tablesCreated.join(', ')}`));
        }
        if (repairResult.indexesCreated.length > 0) {
          console.log(Theme.colors.success(`✓ Created ${repairResult.indexesCreated.length} indexes`));
        }
        console.log(Theme.colors.success('✓ Database tables initialized'));
        return { success: true };
      } else {
        const errorMsg = repairResult.errors.join('; ');
        console.log(Theme.colors.error(`❌ Schema repair failed: ${errorMsg}`));
        return {
          success: false,
          error: errorMsg
        };
