...
export class PostgreSQLAdapter extends DatabaseAdapter {
  ...
  async migrate(): Promise<void> {
    ...
    try {
      ...
      const possiblePaths = [
        join(__dirname, '../../src/database/schema.postgres.sql'), // Docker path
        join(__dirname, '../schema.postgres.sql'), // Local compiled path
        join(process?.cwd(), 'src/database/schema.postgres.sql'), // From project root
      ];
  ...
  async addProjectPath(projectId: string, path: string, pathType: 'primary' | 'alias' | 'historical'): Promise<ProjectPath> {
    if (!this.pool) throw new Error('Database not initialized');
  ...
  async updateProjectPath(projectId: string, oldPath: string, newPath: string): Promise<void> {
    if (!this.pool) throw new Error('Database not initialized');
  ...
  async deactivateProjectPath(projectId: string, path: string): Promise<void> {
    if (!this.pool) throw new Error('Database not initialized');
  ...
  async getProjectByAnyPath(path: string): Promise<Project | null> {
    return this?.getProject(path); // Same implementation for now
  }

  async getProjectPaths(projectId: string): Promise<ProjectPath[]> {
    if (!this.pool) throw new Error('Database not initialized');
  ...
  async saveInitializationProgress(progress: Omit<InitializationProgress, 'id' | 'createdAt' | 'updatedAt'>): Promise<InitializationProgress> {
    if (!this.pool) throw new Error('Database not initialized');
  ...
  async getInitializationProgress(projectPath: string): Promise<InitializationProgress | null> {
    if (!this.pool) throw new Error('Database not initialized');
  ...
  async updateInitializationProgress(resumeToken: string, updates: Partial<InitializationProgress>): Promise<InitializationProgress> {
    if (!this.pool) throw new Error('Database not initialized');
    ...
    Object.entries(updates)?.forEach(([key, value]) => {
      if (key === 'id' || key === 'createdAt' || key === 'projectPath') return;
  ...
  async saveDetectedPattern(pattern: Omit<DetectedPattern, 'id' | 'createdAt' | 'updatedAt'>): Promise<DetectedPattern> {
    if (!this.pool) throw new Error('Database not initialized');
  ...
  async getDetectedPatterns(projectPath: string, patternType?: string): Promise<DetectedPattern[]> {
    if (!this.pool) throw new Error('Database not initialized');
  ...
  async updatePatternStatus(id: string, status: 'detected' | 'validated' | 'rejected'): Promise<void> {
    if (!this.pool) throw new Error('Database not initialized');

    try {
      await this.pool?.query(
        'UPDATE detected_patterns SET status = $1, updated_at = NOW() WHERE id = $2',
        [status, id]
      );
    } catch (error) {
  ...
  async getQuestionnaireResponses(projectPath: string, category?: string): Promise<QuestionnaireResponse[]> {
    if (!this.pool) throw new Error('Database not initialized');
  ...
  async saveAnalysisResult(analysisResult: Omit<AnalysisResult, 'id' | 'createdAt'>): Promise<AnalysisResult> {
    if (!this.pool) throw new Error('Database not initialized');
  ...
  async getAnalysisResults(projectPath: string, analysisType?: string, fileHash?: string): Promise<AnalysisResult[]> {
    if (!this.pool) throw new Error('Database not initialized');
  ...
  async getSystemConfig(key: string): Promise<any> {
    ...
    try {
      const result = await this.pool?.query(
        'SELECT config_value FROM system_config WHERE config_key = $1 AND is_global = true',
        [key]
      );
  ...
  async setSystemConfig(key: string, value: any, projectId?: string): Promise<void> {
    if (!this.pool) throw new Error('Database not initialized');
  ...
  async recordOperationMetrics(operation: string, projectId: string | null, durationMs: number, success: boolean, error?: string, metadata?: Record<string, any>): Promise<void> {
    if (!this.pool) throw new Error('Database not initialized');
  ...
  async cleanupExpiredResumeStates(): Promise<number> {
    if (!this.pool) throw new Error('Database not initialized');
  ...
  async archiveOldAnalysisResults(olderThanDays: number): Promise<number> {
    if (!this.pool) throw new Error('Database not initialized');
  ...
  private mapRowToProjectPath(row: any): ProjectPath {
    return {
      id: row.id,
  ...
  private mapRowToInitializationProgress(row: any, projectPath: string): InitializationProgress {
    return {
      id: row.id,
  ...
  private mapRowToDetectedPattern(row: any, projectPath: string): DetectedPattern {
    return {
      id: row.id,
      projectPath: projectPath,
  ...
  private mapRowToQuestionnaireResponse(row: any, projectPath: string): QuestionnaireResponse {
    return {
      id: row.id,
      projectPath: projectPath,
  ...
  private mapRowToAnalysisResult(row: any, projectPath: string): AnalysisResult {
    return {
      id: row.id,
      projectPath: projectPath,
