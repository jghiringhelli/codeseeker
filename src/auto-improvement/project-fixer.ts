...
export class ProjectFixer {
  ...
  async analyzeAndFix(options: ProjectFixerOptions): Promise<ProjectAnalysisReport> {
  ...
  private async validateProject(projectPath: string): Promise<void> {
    try {
      ...
      throw new Error(`Invalid project path: ${projectPath} - ${(error as Error).message}`);
    }
  }
  ...
  private async analyzeProject(projectPath: string): Promise<ProjectMetrics> {
  ...
  private async applyFixes(options: ProjectFixerOptions, metrics: ProjectMetrics): Promise<FixResult[]> {
  ...
  private calculateImprovement(before: ProjectMetrics, after: ProjectMetrics): ProjectMetrics {
    return {
  ...
  private countIssues(metrics: ProjectMetrics): number {
    let issues = 0;
    if (metrics.duplicateLines > 0) issues++;
    if (metrics.scatteredConfigs > 0) issues++;
    if (metrics.circularDependencies > 0) issues++;
    if (metrics.securityIssues > 0) issues++;
    if (metrics.performanceIssues > 0) issues++;
    if (metrics.architectureViolations > 0) issues++;
    return issues;
  }
  ...
  private async generateRecommendations(projectPath: string, metrics: ProjectMetrics): Promise<string[]> {
  ...
  private generateNextSteps(fixes: FixResult[], metrics: ProjectMetrics): string[] {
  ...
  private async generateReportFile(report: ProjectAnalysisReport, outputPath: string): Promise<void> {
  ...
  private generateMarkdownReport(report: ProjectAnalysisReport): string {
