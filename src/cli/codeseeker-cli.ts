...
/**
 * CodeSeekerCLI - SOLID Principles Implementation
...
export class CodeSeekerCLI {
  ...
  constructor() {
    ...
    this.historyDir = path.join(os.homedir(), '.codeseeker', 'history');
    this.ensureHistoryDir();

    // Determine project-specific history file based on current working directory
    const userCwd = process.env.CODESEEKER_USER_CWD || process.env.CODESEEKER_USER_CWD || process.cwd();
    ...
    this.rl = readline.createInterface({
      ...
      prompt: Theme.colors.prompt('codeseeker>'),
      historySize: CodeSeekerCLI.MAX_HISTORY_SIZE,
  ...
  async startSilent(): Promise<void> {
    try {
      ...
      // Get user's original working directory (set by bin/codeseeker.js)
      const userCwd = process.env.CODESEEKER_USER_CWD || process.cwd();

      // Load environment variables from user's working directory (not CodeSeeker installation)
  ...
  async start(): Promise<void> {
    try {
      ...
      console.log(Theme.colors.primary('\n🎯 CodeSeeker CLI is ready! Type /help for commands or ask questions directly.'));
  ...
  private setupEventHandlers(): void {
    ...
    this.rl.on('close', async () => {
      ...
      console.log(Theme.colors.primary('\n👋 Goodbye! Thank you for using CodeSeeker.'));
  ...
  /**
   * Auto-detect CodeSeeker project silently (no output)
   */
  ...
  /**
   * Auto-detect CodeSeeker project - Delegated to ProjectManager (SOLID)
   */
  private async autoDetectProject(): Promise<void> {
    ...
    if (projectConfig) {
      ...
      // Load CODESEEKER.md instructions
      try {
        const instructionsSummary = await this.context.instructionService.getInstructionsSummary(userCwd);
        if (instructionsSummary.length > 1 || instructionsSummary[0] !== 'No CODESEEKER.md instructions found') {
      ...
      console.log(Theme.colors.warning('\n⚠ No CodeSeeker project found in current directory'));
      console.log(Theme.colors.muted('Run "/init" to initialize this directory as a CodeSeeker project'));
    }
  ...
  private async checkDatabaseConnections(): Promise<void> {
    try {
      ...
      const useEmbedded = isUsingEmbeddedStorage();

      if (useEmbedded) {
        // Embedded mode - show lightweight local storage info
        console.log(Theme.colors.muted('\n📦 Storage: Embedded mode (SQLite + MiniSearch)'));
        console.log(Theme.colors.success('   ✓ Local storage ready - no external databases needed'));
        return;
      }
      ...
      console.log(Theme.colors.muted('   CodeSeeker will work with limited functionality. Use /init to set up databases.'));
    }
  }
    ...
    try {
      if (fs.existsSync(this.historyFile)) {
        ...
        this.commandHistory = content
          ...
          .slice(-CodeSeekerCLI.MAX_HISTORY_SIZE);
      }
    } catch {
  ...
  private addToHistory(command: string): void {
    ...
    // Trim to max size
    if (this.commandHistory.length > CodeSeekerCLI.MAX_HISTORY_SIZE) {
      this.commandHistory = this.commandHistory.slice(-CodeSeekerCLI.MAX_HISTORY_SIZE);
    }
  ...
  private getProjectHistoryFile(projectPath: string): string {
    try {
      ...
      return path.join(os.homedir(), '.codeseeker_history');
    }
  }
...
export async function main(): Promise<void> {
  ...
  // Check for MCP server mode: codeseeker serve --mcp
  ...
  // Check for install command: codeseeker install --copilot|--cursor|etc
  const isInstallCommand = args[0] === 'install';
  if (isInstallCommand) {
    const { InstallCommandHandler } = await import('./commands/handlers/install-command-handler');
    const { CommandServiceFactory } = await import('../core/factories/command-service-factory');
    const factory = CommandServiceFactory.getInstance();
    const context = factory.createCommandContext();
    const handler = new InstallCommandHandler(context);
    const installArgs = args.slice(1).join(' ');
    const result = await handler.handle(installArgs);
    process.exit(result.success ? 0 : 1);
  }

  if (args.includes('--help') || args.includes('-h')) {
    console.log(`
${Theme.colors.primary('CodeSeeker Interactive CLI - Intelligent Code Assistant')}

${Theme.colors.secondary('Usage:')}
  codeseeker [options] [command]

${Theme.colors.secondary('Commands:')}
  install <ide>         Configure MCP for your IDE (--copilot, --cursor, --visual-studio)
...
${Theme.colors.secondary('Examples:')}
  codeseeker                         Start interactive mode in current directory
  codeseeker install --copilot       Configure VS Code + GitHub Copilot for this project
  codeseeker install --cursor -g     Configure Cursor globally (all projects)
  codeseeker -p /path/to/proj        Start with specific project path
  codeseeker -c "analyze main"       Execute single command and exit
  codeseeker serve --mcp             Start MCP server for Claude Desktop
`);
    return;
  }
  ...
  if (args.includes('--version') || args.includes('-V')) {
    ...
    try {
      ...
      console.log('1.3.0'); // Fallback
    }
    return;
  }

  const cli = new CodeSeekerCLI();
...
process.on('unhandledRejection', (error: unknown) => {
  ...
  console.error('CodeSeeker will continue running, but this issue should be fixed.');
...
if (require.main === module) {
  main().catch((error: unknown) => {
    const errorMessage = error instanceof Error ? error.message : String(error);
    console.error(Theme.colors.error(`\n❌ Failed to start CodeSeeker CLI: ${errorMessage}`));
    process.exit(1);
  });
}

export default CodeSeekerCLI;