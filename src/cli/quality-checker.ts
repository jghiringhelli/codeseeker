...
} from './quality/interfaces/index';
import { CompilationService } from './quality/services/compilation-service';
import { TestingService } from './quality/services/testing-service';
import { SecurityService } from './quality/services/security-service';
import { ArchitectureService } from './quality/services/architecture-service';
import { QualityScoreCalculator } from './quality/services/quality-score-calculator';
...
export class QualityChecker {
  ...
  constructor(
    ...
    private architectureService?: IArchitectureService,
    private scoreCalculator?: IQualityScoreCalculator
  ) {
    this.projectRoot = projectRoot || process.cwd();
    this.db = db || new DatabaseConnections();

    // Initialize services with dependency injection
    this.compilationService = this.compilationService || new CompilationService(this.projectRoot);
    this.testingService = this.testingService || new TestingService(this.projectRoot);
    this.securityService = this.securityService || new SecurityService(this.projectRoot);
    this.architectureService = this.architectureService || new ArchitectureService(this.projectRoot);
    this.scoreCalculator = this.scoreCalculator || new QualityScoreCalculator();
  }
  ...
  async check(result: any): Promise<any> {
    ...
    try {
      ...
      return this.generateFailedResult(`Quality check failed: ${(error as Error).message}`);
    }
  }
  ...
  async runAllChecks(subTaskResults: SubTaskResult[]): Promise<QualityCheckResult> {
    ...
    try {
      ...
      const blockers = recommendations.filter(rec =>
        rec.includes('Fix') && (rec.includes('before proceeding') || rec.includes('critical'))
      );
      ...
      return this.generateFailedResult(`Quality validation failed: ${(error as Error).message}`);
    }
  }
  ...
  private normalizeInput(result: any): SubTaskResult[] {
  ...
  private logQualitySummary(result: QualityCheckResult): void {
