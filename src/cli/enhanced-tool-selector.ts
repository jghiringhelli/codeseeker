...
import { ToolBundleSystem, ToolBundle, BundleSelectionContext } from './tool-bundle-system';
...
export class EnhancedToolSelector {
   ...
   */
  public selectTools(context: SelectionContext): ToolSelectionResult {
   ...
   */
  private selectIndividualTools(context: SelectionContext): ToolDefinition[] {
    const { userQuery, intent, codebaseSize } = context;
    const queryLower = userQuery.toLowerCase();
    
    const candidates: Array<{ tool: ToolDefinition; score: number }> = [];
   ...
   */
  private optimizeForTokenBudget(tools: ToolDefinition[], maxTokens: number): ToolDefinition[] {
   ...
   */
  public getAvailableBundles(): ToolBundle[] {
   ...
   */
  public createToolBundle(bundle: Omit<ToolBundle, 'id'>): string {
