...
export interface ImportRelation {
  ...
  specifiers: string[]; // what is imported (functions, classes, etc.)
}
...
export class CompleteGraphBuilder {
  ...
  async buildCompleteGraph(): Promise<{nodes: FileNode[], relationships: GraphRelationship[]}> {
  ...
  private async analyzeFileNode(filePath: string): Promise<void> {
    try {
  ...
  private determineFileType(filePath: string, content: string): 'file' | 'module' | 'config' | 'test' | 'documentation' {
  ...
  private extractImports(content: string, filePath: string): ImportRelation[] {
  ...
  private extractVariables(content: string, filePath: string): VariableInfo[] {
  ...
  private detectFilePatterns(content: string, filePath: string): PatternInfo[] {
  ...
  private extractMethodsFromClass(content: string, classStart: number, className: string): FunctionInfo[] {
  ...
  private extractFunctionCalls(content: string, functionStart: number, functionName: string): string[] {
  ...
  private findFileContainingClass(className: string): string | undefined {
    for (const [filePath, fileNode] of this.fileNodes) {
  ...
  private findFileContainingInterface(interfaceName: string): string | undefined {
    for (const [filePath, fileNode] of this.fileNodes) {
  ...
  private isConfigurationRelated(configNode: FileNode, targetNode: FileNode): boolean {
  ...
  private findTestedFile(testPath: string, testNode: FileNode): string | undefined {
  ...
  private calculateSemanticSimilarity(node1: FileNode, node2: FileNode): number {
