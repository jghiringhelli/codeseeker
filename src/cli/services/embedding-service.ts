...
export class EmbeddingService {
  ...
  constructor(config?: Partial<EmbeddingConfig>) {
    ...
    const envModel = process.env.EMBEDDING_MODEL as 'Xenova/all-MiniLM-L6-v2' | 'text-embedding-ada-002' | 'text-embedding-3-small' | 'text-embedding-3-large' | 'local';
   ...
   */
  async generateEmbedding(content: string, filePath: string): Promise<number[]> {
   ...
   */
  private async generateXenovaEmbedding(content: string, filePath: string): Promise<number[]> {
    try {
      ...
      if (!this.xenovaExtractor) {
        ...
        this.xenovaExtractor = await pipeline(
          'feature-extraction',
          this.config.model, // Default: 'Xenova/all-MiniLM-L6-v2'
          {
