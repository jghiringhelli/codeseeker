...
import { LocalEmbeddingProvider } from '../content-processing/content-processor';
...
export class DuplicateCodeDetector {
  ...
  private async extractCodeChunks(projectPath: string, config: DeduplicationOptions): Promise<CodeChunk[]> {
    ...
    const { ProjectFileScanner } = await import('../file-scanner/project-file-scanner');
    const { TreeSitterSemanticBuilder } = await import('../semantic-graph/tree-sitter-semantic-builder');
