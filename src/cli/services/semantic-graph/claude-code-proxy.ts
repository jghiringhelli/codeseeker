...
export class ClaudeCodeProxy {
  ...
  async analyzeFile(file: FileInfo): Promise<ClaudeAnalysisResult> {
    ...
    try {
      ...
      const result = await this.executeClaudeCode(analysisPrompt, file.path);
...
Focus on accuracy and only include relationships you're confident about.`;
  ...
  private async executeClaudeCode(prompt: string, filePath: string): Promise<string> {
    // Try different Claude Code invocation methods
    const commands = [
      `${this.claudeCommand} --analyze "${filePath}"`,
      `${this.claudeCommand} -f "${filePath}" --prompt "${prompt}"`,
      `echo "${prompt}" | ${this.claudeCommand} --stdin`
    ];

    for (let i = 0; i < commands.length && i <= this.maxRetries; i++) {
      try {
        const { stdout, stderr } = await execAsync(commands[i % commands.length], {
        ...
        console.warn(`Claude Code command ${i + 1} failed: ${error.message}`);

        if (i === this.maxRetries) {
          throw new Error(`All Claude Code invocation methods failed`);
        }
      }
    }
