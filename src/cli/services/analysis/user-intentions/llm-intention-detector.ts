...
export type AssumptionCategory =
  | 'scope' | 'approach' | 'technology' | 'architecture' | 'data' | 'integration'
  | 'behavior' | 'format' | 'timeline' | 'quality' | 'performance' | 'security'
  | 'testing' | 'deployment' | 'maintenance' | 'compatibility' | 'resources';
...
export interface AssumptionDetail {
  ...
  userSelectable: boolean; // Whether user can choose from alternatives
}

export interface AmbiguityDetail {
  id: string; // Unique identifier for user selection
  area: string;
  reason: string;
  clarificationNeeded: string;
  options: string[]; // User-selectable options
  allowCustomInput: boolean; // Whether user can provide custom answer
...
export interface UserClarification {
  ...
  customInput?: string; // Custom user input
  timestamp: number;
}
...
export interface ClarifiedIntentionAnalysis extends IntentionAnalysis {
  ...
  finalInstructions: string; // Combined instructions for LLM execution
}

export class LLMIntentionDetector {
  ...
  async analyzeIntention(userInput: string): Promise<IntentionAnalysis | null> {
    if (!this.enableLLM) {
      return null;
    }
    ...
    try {
      ...
      const response = await CommandProcessor.executeClaudeCode(prompt, {
...
`;
  ...
  private parseResponse(response: string): IntentionAnalysis {
    try {
      ...
      parsed.assumptions = parsed.assumptions.map((assumption: any) => ({
      ...
      parsed.ambiguities = parsed.ambiguities.map((ambiguity: any, index: number) => ({
