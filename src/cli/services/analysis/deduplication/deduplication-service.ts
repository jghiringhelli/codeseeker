...
import { EmbeddingService, MethodEmbedding, ClassEmbedding } from '../../data/embedding/embedding-service';
...
export class DeduplicationService {
  ...
  constructor() {
    ...
    this.embeddingService = new EmbeddingService({ granularMode: true });
  ...
  private async findMethodDuplicates(
    ...
    for (const method of methods) {
      ...
      const similarMethods = await this.embeddingService.findSimilarMethods(
        method,
  ...
  private async findClassDuplicates(
    ...
    for (const classItem of classes) {
      ...
      const similarClasses = await this.embeddingService.findSimilarClasses(
        classItem,
        0.7 // Lower threshold to catch more potential duplicates
      );
