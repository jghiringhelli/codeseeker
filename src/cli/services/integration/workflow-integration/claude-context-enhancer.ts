...
import { ProjectFileScanner } from '../../monitoring/file-scanning/project-file-scanner';
import { SemanticGraphService } from '../../data/semantic-graph/semantic-graph';
import { ContentProcessor } from '../../data/content-processing/content-processor';
import { SemanticSearchService } from '../../search/semantic-search';
...
export class ClaudeContextEnhancer {
  ...
  constructor(config?: Partial<ClaudeContextConfig>, databaseClient?: any) {
    ...
    // Initialize unified search service using factory
    const searchFactory = SearchServiceFactory.getInstance();
    this.searchService = searchFactory.createSemanticSearchService();
  }
  ...
  async initialize(projectPath: string): Promise<void> {
    ...
    try {
      ...
      const semanticGraph = await this.semanticGraphService.buildGraphFromFiles(scanResult.files);
      ...
      for (const result of contentResults) {
        if (result.chunks.length > 0) {
          // Content is processed by the semantic search service internally
        }
      }
  ...
  private async analyzeFileRelevance(request: ContextEnhancementRequest, searchResults: any[]) {
    ...
    for (const result of searchResults) {
      ...
      existing.score = Math.max(existing.score, result.relevanceScore);
      existing.reasons.push(`Semantic match (${result.relevanceScore.toFixed(2)})`);
  ...
  private generateEnhancedPrompt(
    ...
    if (searchResults.length > 0) {
      ...
      searchResults.slice(0, 3).forEach((result, index) => {
        prompt += `\n#### Snippet ${index + 1} (similarity: ${result.relevanceScore.toFixed(2)})\n`;
  ...
  private calculateContextSummary(relevantFiles: any[], searchResults: any[], processingTime: number) {
    ...
    const confidenceScore = searchResults.length > 0
      ? searchResults.reduce((sum, result) => sum + result.relevanceScore, 0) / searchResults.length
      : 0;
  ...
  async getStats() {
    ...
    const indexStats = await this.searchService.getStats();
