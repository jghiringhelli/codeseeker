...
export class GitWorkflowService implements IGitWorkflowService {
  ...
  async createFeatureBranch(workflowId: string, description: string): Promise<string> {
    ...
    try {
      ...
      const branchName = `codeseeker/${workflowId}/${sanitizedDescription}`;
  ...
  async finalizeChanges(
    ...
    this.logger.info('🔚 Finalizing changes...');
  ...
  async rollbackChanges(workflowId: string): Promise<void> {
    this.logger.info('↩️ Rolling back changes...');
  ...
  async commitChanges(branch: string, message: string, files: string[]): Promise<void> {
    ...
    try {
      ...
      const commitMessage = `${message}\n\nGenerated by CodeSeeker Workflow\nBranch: ${branch}\nFiles: ${files.join(', ')}`;
  ...
  private async commitAndMergeChanges(
    ...
    try {
      ...
      await this.commitChanges(
        branch,
        'CodeSeeker workflow implementation - quality checks passed',
        filesModified
      );
  ...
  private async handleQualityFailure(
    ...
    await this.commitChanges(
      branch,
      `CodeSeeker workflow - quality issues found (score: ${quality.overallScore}%)`,
      filesModified
    );
  ...
  async ensureCleanWorkingDirectory(): Promise<void> {
    ...
    if (status.hasUncommittedChanges) {
      ...
      await this.gitManager.stashChanges('CodeSeeker auto-stash before workflow');
    }
  }
}