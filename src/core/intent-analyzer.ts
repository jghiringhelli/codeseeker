...
export class IntentAnalyzer {
  ...
  async analyzeIntent(request: IntentAnalysisRequest): Promise<ProcessedIntent> {
  ...
  private buildIntentAnalysisPrompt(request: IntentAnalysisRequest): string {
...
Provide only the JSON response, no additional text.`;
  ...
  private parseIntentResponse(response: string, originalQuery: string): ProcessedIntent {
    try {
  ...
  private adjustForProjectContext(intent: ProcessedIntent, context?: any): ProcessedIntent {
  ...
  private generateFallbackIntent(query: string): ProcessedIntent {
    return {
      intention: 'general_analysis',
      complexity: 'simple',
      estimatedFiles: 3,
