...
export class RedisCacheAdapter<T> {
   ...
   */
  async get(key: string, contentHash?: string): Promise<RedisCacheEntry<T> | null> {
    if (!this.isConnected) {
    ...
    try {
      ...
      const entry: RedisCacheEntry<T> = JSON.parse(cachedData);
  ...
  private createRedisKey(key: string): string {
  ...
  private async updateAccessStats(redisKey: string, entry: RedisCacheEntry<T>): Promise<void> {
    try {
...
export class RedisCacheFactory {
  static createCache<T>(prefix: string): RedisCacheAdapter<T> {
    return new RedisCacheAdapter<T>(prefix);
  }
