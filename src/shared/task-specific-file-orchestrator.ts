...
export interface FileTask {
  ...
  claudeInstructions: string; // Specific instructions for Claude Code
  validationCriteria: string[]; // How to validate this specific change
}
...
export class TaskSpecificFileOrchestrator {
  ...
  async orchestrateRequest(
    ...
    try {
      ...
      await this.branchManager.createSnapshot(
        ...
        'Pre-execution state',
        validationPassed,
        fileTasks.map(t => t.filePath)
      );

      // 5. PHASE 5: Execute File Tasks (if not dry run)
      let completedTasks: FileTask[] = [];
      let failedTasks: FileTask[] = [];
      ...
      if (!options.dryRun) {
        ...
        await this.branchManager.createSnapshot(
          ...
          completedTasks.map(t => t.filePath)
        );
      }
      ...
      if (!options.skipCycles && !options.dryRun) {
        ...
        postValidationPassed = await this.runPostExecutionValidation(
          projectPath,
          completedTasks.map(t => t.filePath)
        );
      ...
      if (!options.dryRun && completedTasks.length > 0 && postValidationPassed) {
        ...
        integrationResult = await this.postExecutionIntegration.runCompleteIntegration(
          featureBranch,
          completedTasks.map(t => t.filePath),
          {
   ...
   */
  private convertToFileTasks(impactResult: ImpactAnalysisResult, userRequest: string): FileTask[] {
   ...
   */
  private generateClaudeInstructions(file: AffectedFile, userRequest: string): string {
    ...
    switch (file.fileType) {
      ...
      case 'config':
        instructions.push(
          ...
          `• Update configuration to support: ${userRequest}`,
          `• Preserve existing settings unless they conflict`,
      ...
      case 'documentation':
        instructions.push(
          ...
          `• Document changes related to: ${userRequest}`,
          `• Update relevant sections without changing unrelated content`,
      ...
      case 'test':
        instructions.push(
          ...
          `• ${file.changeType === 'create' ? 'Create comprehensive tests' : 'Update existing tests'} for: ${userRequest}`,
      ...
      case 'deployment':
        instructions.push(
          'DEPLOYMENT FILE INSTRUCTIONS:',
          `• Update deployment configuration for: ${userRequest}`,
    ...
    instructions.push(
      ...
      'VALIDATION:',
      `• Priority: ${file.priority}`,
      `• Complexity: ${file.estimatedComplexity}/10`,
   ...
   */
  private generateValidationCriteria(file: AffectedFile, userRequest: string): string[] {
   ...
   */
  private async executeFileTasks(
    tasks: FileTask[],
    featureBranch: FeatureBranch
  ): Promise<{ completed: FileTask[]; failed: FileTask[] }> {
    const completed: FileTask[] = [];
    const failed: FileTask[] = [];
   ...
   */
  private async executeFileTask(task: FileTask): Promise<boolean> {
   ...
   */
  private async performIntelligentRollback(
    featureBranch: FeatureBranch,
    completedTasks: FileTask[],
    failedTasks: FileTask[]
  ): Promise<void> {
   ...
   */
  private sortTasksByPriorityAndDependencies(tasks: FileTask[]): FileTask[] {
  ...
  private generateSuccessMessage(
    ...
    if (integrationResult) {
      ...
      if (integrationResult.compilationFixed) integrationDetails.push('compilation fixed');
      if (integrationResult.testsFixed) integrationDetails.push('tests fixed');
      if (integrationResult.changesCommitted) integrationDetails.push('changes committed');
      if (integrationResult.branchMerged) integrationDetails.push('branch merged');
      if (integrationResult.documentationUpdated) integrationDetails.push('docs updated');
   ...
   */
  private calculateActualTime(startTime: number): string {
